...
static pthread_mutex_t locks_files;
...
int state_init(tfs_params params) {
    ...
    locks_inodes = malloc(INODE_TABLE_SIZE * sizeof(pthread_rwlock_t));

    if (!inode_table || !freeinode_ts || !fs_data || !free_blocks ||
        !open_file_table || !free_open_file_entries || !locks_inodes) {
        return -1; // allocation failed
    }

    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        pthread_rwlock_init(&locks_inodes[i], NULL);
        freeinode_ts[i] = FREE;
    }
    ...
    pthread_mutex_init(&locks_files, NULL);
...
int state_destroy(void) {
    ...
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        pthread_rwlock_destroy(&locks_inodes[i]);
    }

    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_rwlock_destroy(&open_file_table[i].lock);
    }
    pthread_mutex_destroy(&locks_files);
...
int add_dir_entry(inode_t *inode, char const *sub_name, int sub_inumber) {
    ...
    for (int j = 0; j < INODE_TABLE_SIZE; j++) {
        if (&inode_table[j] == inode) {
            inumber = j;
            pthread_rwlock_rdlock(&locks_inodes[j]);
            break;
        }
    }
    if (inumber == -1) {
    ...
    for (size_t i = 0; i < MAX_DIR_ENTRIES; i++) {
        if (dir_entry[i].d_inumber == -1) {
            ...
            pthread_rwlock_unlock(&locks_inodes[inumber]);
...
/**
 * Allocate a newath para o target no bloco de dados então não está vazio data
 * block.
 *
...
int data_block_alloc(void) {
    pthread_rwlock_rdlock(&locks_blocks);
    for (size_t i = 0; i < DATA_BLOCKS; i++) {
        if (i * sizeof(allocation_state_t) % BLOCK_SIZE == 0) {
            insert_delay(); // simulate storage access delay to free_blocks
        }
        if (free_blocks[i] == FREE) {
            pthread_rwlock_unlock(&locks_blocks);
            pthread_rwlock_wrlock(&locks_blocks);
            if (free_blocks[i] == FREE) {
                free_blocks[i] = TAKEN;
                pthread_rwlock_unlock(&locks_blocks);
                return (int)i;
...
int add_to_open_file_table(int inumber, size_t offset) {
    pthread_mutex_lock(&locks_files);
    ...
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            ...
            pthread_mutex_unlock(&locks_files);
